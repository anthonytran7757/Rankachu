...
export const PokeInfo = (props: PokemonInfoProps) => {
  ...
  const [overallVoteToggle, setOverallVoteToggle] = React.useState<boolean>(
    true
  );
  const [legendaryVoteToggle, setLegendaryVoteToggle] = React.useState<boolean>(
    true
  );
  ...
  React.useEffect(() => {
    ...
    async function retrieveSpeciesDetails() {
      ...
      let tempSpecData: specInfoData = {
        ...
        eggGroupTwo: "",
        genIntro: parseInt(
          data.generation.url.replace(GEN_URL, "").slice(0, -1)
        ),
  ...
  const renderTypeImgs = () => {
    let typeImgs = pokeData?.types.map((type) => (
      <img
        className="typeIMG"
        alt="vanity"
        src={`../assets/elements/${type.toLowerCase()}.png`}
      />
    ));
    return typeImgs;
  };
    ...
    if (pokeData?.secondRegAbility) {
      return (
        <div className="singleField">
          ...
          <p className="content">
            {pokeData.regularAbility} or {pokeData.secondRegAbility}
          </p>
        </div>
      );
    } else {
  ...
  const renderStats = () => {
    let statHeader = STAT_LABELS.map((header) => {
      return (
        <Col className="statContent" xs={2}>
    ...
    return (
      <Grid className="grid">
        <Row xs={12}>
          <h4 className="gridHeader">Stats</h4>
        </Row>
        <Row>{statHeader}</Row>
        <Row>{pokeStats}</Row>
      </Grid>
  ...
  const renderEVs = () => {
    ...
    return (
      <Grid className="grid">
        <Row xs={12}>
          <h4 className="gridHeader">EVs</h4>
        </Row>
        <Row>{statHeader}</Row>
        <Row>{EVs}</Row>
      </Grid>
    ...
    if (specData?.eggGroupTwo === "") {
      ...
      return (
        <div className="singleField">
          <h4 className="title">Egg Group: </h4>
          <p className="content">
            {specData?.eggGroupOne} or {specData?.eggGroupTwo}
          </p>
        </div>
      );
    }
  ...
  return (
    <div hidden={hidden} className="pokeInfo">
      <Panel className="card" shaded bordered bodyFill>
        ...
        />
        <h2>
          {pokeData?.name}: #{pokeData?.dexNum}
        </h2>
        <img className="artwork" src={pokeData?.officialArt} alt="vanity" />
        <div className="types">{renderTypeImgs()}</div>
        {renderRegAbility()}
        <div className="singleField">
        ...
        <div className="singleField">
          <h4 className="title">Catch Rate: </h4>
          <p className="content">{specData?.catchRate}</p>
        </div>
        {renderEggGroup()}
        {renderStats()}
        {renderEVs()}
